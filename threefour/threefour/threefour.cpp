...
//функция добавления студента в динамический массив структур
void add_student(person*& group, int& amount, person student) {
    group[amount] = student;                                        
    amount++;
}
//функция удаления студента по индексу из динамического массива структур
...
//функция вывода всех студентов на консоль
...
//функция сохранения в файл студентов из массива структур 
void save_to_file(person* group, int amount, const char* file_name) {
    ofstream file(file_name);
    for (int i = 0; i < amount; i++) {
        file << group[i].name << " " << group[i].age << " " << group[i].sex << " " << group[i].course << " " << group[i].mark << endl;
    } 
    file.close();
}
//функция сохранения в файл отредактирвоанного списка студентов(из условия)
...
//функция вывода на консоль студентов отредактированного списка 
...
int main() {
    ...
    save_to_file(group, amount, "students.txt");
    ...
    save_older_students(group, amount, course, age_limit, "older_students.txt");
